...
class CircuitResistanceCalculator(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Calcul Rezistență Circuit")
        self.init_ui()
    ...
    def init_ui(self):
        ...
        self.result_label = ttk.Label(self, text="Rezistență circuit:")
    ...
    def calculate(self):
        try:
            ...
            if R_total > 1000:
                self.result_label.config(text=f"Rezistență: {R_total/1000:.2f} kΩ")
            else:
                self.result_label.config(text=f"Rezistență: {R_total:.2f} Ω")
        except Exception:
            self.result_label.config(text="Eroare: Verificați inputul.")

if __name__ == "__main__":
    app = CircuitResistanceCalculator()
    app.mainloop()